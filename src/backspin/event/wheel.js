...
module.exports = async(ctx, id, betType, amount) => {
    let randomResult = generate.generateNumber(0, 36);
    let randomResult2 = generate.generateNumber(0, 36);
    let randomResult3 = generate.generateNumber(0, 36);
    let randomResult4 = generate.generateNumber(0, 36);
    let randomResult5 = generate.generateNumber(0, 36);
    let randomResult6 = generate.generateNumber(0, 36);
    let randomResult7 = generate.generateNumber(0, 36);
    let randomResult8 = generate.generateNumber(0, 36);
    let randomResult9 = generate.generateNumber(0, 36);

    await postgresql.request(`SELECT * FROM conversation WHERE id=${ctx.peerId}`).then(async([data]) => {
        console.log(data);
        vk.group.api.messages.send({
            message: `${amount < 10_000_000 ? `✅` : `⛔`}Игрок [id${id}|${await redis.getName(id)}] сделал ставку в Wheel на ${(betType.split('#')[0] === "numbers" ? `${settings.games.wheel[betType.split('#')[0]]} ${betType.split('#')[1]}` : settings.games.wheel[betType])}\nРазмер ставки: ${utils.format(amount)}\nИсход: ${data.result}\nДо конца раунда: ${utils.time(data.time)}`,
            peer_id: 2000000011,
            random_id: 0,
