...
module.exports = async (ctx) => {
    if (ctx.messagePayload?.action.split("@")[0] === 'litewheel') {
        ...
        if (JSON.parse(data).bets_button) {
            betAmount = await ctx.question({
                message: `${await helper.getName(ctx.senderId)}, Введите сумму ставки на ${betType.split("#")[0] == 'numbers' ? `число ${betType.split("#")[1]}` : settings.games.litewheel[betType]} или нажмите на кнопку:`,
            ...
            betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, Введите сумму ставки на ${betType.split("#")[0] == 'numbers' ? `число ${betType.split("#")[1]}` : settings.games.litewheel[betType]}:`);
        }
        ...
        if (gameData.gamemode !== "litewheel") return;
        ...
        ctx.send({
            message: `${await helper.getName(ctx.senderId)}, успешная ставка ${utils.format(betAmount.text)} VKC на ${(betType.split('#')[0] === "numbers" ? `${settings.games.litewheel[betType.split('#')[0]]} ${betType.split('#')[1]}` : settings.games.litewheel[betType])}`
        });

        require('../../../backspin/event/litewheel')(ctx, ctx.senderId, betType, betAmount.text);
    }
};