...
module.exports = async (ctx) => {
    if (ctx.messagePayload?.action.split("@")[0] === "x100") {
        ...
        betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, введи сумму ставки на ${settings.games.x100[betType]} или нажми на кнопку:`, {
        ...
        betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, введи сумму ставки на ${settings.games.x100[betType]}:`);
    }
        ...
        if (gameData.gamemode !== "x100") return;
        ...
        ctx.send({
            message: `${await helper.getName(ctx.senderId)}, успешная ставка ${utils.format(betAmount.text)} VKC на ${settings.games.x100[betType]}`
        });

        require('../../../backspin/event/x100')(ctx, ctx.senderId, betType, betAmount.text);
    }
};