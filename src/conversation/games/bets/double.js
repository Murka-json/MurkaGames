...
module.exports = async (ctx) => {
    if (ctx.messagePayload?.action.split("@")[0] === "double") {
        ...
        let checkBetHistory = await postgresql.request(`SELECT * FROM bets WHERE playerId='${ctx.senderId}' AND gameId='${ctx.peerId}' AND type!='${betType}'`).catch((err) => { throw err; });
        if (checkBetHistory.length >= 2) return ctx.send(`${await helper.getName(ctx.senderId)}, за один раунд можно поставить лишь 2 ставки.`);
        ...
        betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, введи сумму ставки на ${settings.games.double[betType]} или нажми на кнопку:`, {
        ...
        betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, введи сумму ставки на ${settings.games.double[betType]}:`);
    }
        ...
        if (gameData.gamemode !== "double") return;
        ...
        if (betsList.length === 1) {
            await postgresql.request(`UPDATE bets SET amount=round(amount+${betAmount.text}, 0) WHERE playerid=${betsList[0].id}`).catch((err) => { throw err; });
        } else {
        ...
        ctx.send({
            message: `${await helper.getName(ctx.senderId)}, успешная ставка ${utils.format(betAmount.text)} VKC на ${settings.games.double[betType]}`
        });

        require('../../../backspin/event/double')(ctx, ctx.senderId, betType, betAmount.text);
    }
};