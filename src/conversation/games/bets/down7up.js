...
module.exports = async (ctx) => {
    if (ctx.messagePayload?.action.split("@")[0] === "down7up") {
        ...
        let [bets] = await postgresql.request(`SELECT * FROM bets WHERE playerid=${ctx.senderId}`);
        console.log(bets);
        if(betType == "more" && bets?.type == "less") return ctx.send(`Поставить можно лишь на одну сторону`);
        if(betType == "less" && bets?.type == "more") return ctx.send(`Поставить можно лишь на одну сторону`);
        ...
        betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, введи сумму ставки на ${settings.games.down7up[betType]} или нажми на кнопку:`, {
        ...
        betAmount = await ctx.question(`${await helper.getName(ctx.senderId)}, введи сумму ставки на ${settings.games.down7up[betType]}:`);
    }
        ...
        if (gameData.gamemode !== "down7up") return;
        ...
        ctx.send({
            message: `${await helper.getName(ctx.senderId)}, успешная ставка ${utils.format(betAmount.text)} VKC на ${settings.games.down7up[betType]}`
        });


        require('../../../backspin/event/down7up')(ctx, ctx.senderId, betType, betAmount.text);
    }
};